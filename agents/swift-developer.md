...
5. Add a regression test to prevent recurrence — unless the task owes none (`spine-toolkit:test-authoring`, `## When the task owes no test`) — in the framework `spine-toolkit:test-authoring` picks for that file — the framework of the file you extend, else the project's `- Tests:` value. Its syntax is `test-frameworks`, the section named after that value.
...
- **NEVER reference the current task, phase, EPIC, ticket, fix, PR, or caller** in production code comments. Examples of forbidden patterns:
  ...
  - `// Bug57 fix — null-check before unwrap`
...
- **Acceptable comment shapes:**
  - `/// Shared thumbnail loader. Invariant: all consumers read the same payload to avoid double-decode races.`
