...
- **XcodeBuildMCP** — primary tool for running tests (`test_sim` with `-collect-test-diagnostics never` (rule and fallback: `swift-validator` → Tooling Procedure)), building (`build_sim`), and inspecting build settings. Use it when the orchestrator asks for a Validation step.
...
- Execution via `xcodebuild test` with `-collect-test-diagnostics never` (rule and fallback: `swift-validator` → Tooling Procedure)
