...
- `di-factory` — Factory-specific PR red flags: `Container.shared.foo()` called from domain/service code (Service Locator); factory closure resolves through `Container.shared.bar()` instead of `self.bar()` (breaks test isolation); `@Injected` in a ViewModel, Coordinator or service instead of `init` injection (resolution belongs at the composition edge); `.singleton` on a ViewModel (state shared across screens); `ParameterFactory` combined with `.cached` without `scopeOnParameters` (same instance for different parameters); `register(...)` call in production code outside `autoRegister()` or tests; missing `Container.shared.reset()` in XCTest `setUp` (inter-test pollution), or a test that expects it to clear `.singleton` instances (only `Scope.singleton.reset()` or `@Suite(.container)` isolates them); a test or preview mock registered on a factory whose `.onTest`/`.onPreview`/`.onDebug` context wins over it; `import Factory` instead of `import FactoryKit`; `import FactoryKit` inside an SPM package main target (violates `pkg-spm-design`); name-collisions when two extension files declare same `var name: Factory<…>`
