...
- `xcodebuild` via XcodeBuildMCP (`build_sim`, `test_sim` with `-collect-test-diagnostics never` (rule and fallback: `swift-validator` → Tooling Procedure)) — confirm reproducer builds
...
- `di-factory` — Factory-specific diagnostics: tests influencing each other (missing `Container.shared.reset()` in XCTest `setUp`; `.singleton` instances persisting across `reset()`, which only `Scope.singleton.reset()` clears); a mock registered in a test or preview never used (an active `.onTest`/`.onPreview`/`.onDebug` context wins over `register`); `converting function value … loses global actor 'MainActor'` after moving to Factory 3 (a 2.x `self { @MainActor in … }` closure — put `@MainActor` on the factory variable); UI re-rendering on every event (`@Injected` without `@ObservationIgnored` inside `@Observable`); same instance returned for different `ParameterFactory` arguments (`.cached` without `scopeOnParameters`); test isolation broken when factory closure resolves through `Container.shared.foo()` instead of `self.foo()`; silent registration override across registration files (two `extension Container` files in the app target declaring the same `var name: Factory<…>` — last-loaded wins); `no such module 'Factory'` after moving to Factory 3 (still importing `Factory` instead of `FactoryKit`)
