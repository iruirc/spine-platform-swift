...
**What it is:** Encapsulates a whole UI feature (checkout, onboarding, a media player) with its own UI, behavior, and runtime state.

**Examples:** `CheckoutFeature`, `OnboardingFeature`.
...
**Examples:** `Networking`, `Logging`, `DesignSystem`.
...
**Examples:** `AuthAPI` (the token contract; implementation in `Auth`).
...
- **Cycle breaking:** `Auth` depends on `Networking` for HTTP transport, while `Networking` needs `Auth` to refresh an expired token → both depend on `AuthAPI` (the refresh contract lives there), and the implementation graph no longer cycles.
...
**Examples:** `GameEngine` (Metal), `PurchasesSDK` (StoreKit).
