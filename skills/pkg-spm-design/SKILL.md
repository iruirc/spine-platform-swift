...
**Что это:** Инкапсулирует целую UI-фичу (чекаут, онбординг, медиаплеер) с собственным UI, поведением и runtime-состоянием.

**Примеры:** `CheckoutFeature`, `OnboardingFeature`.
...
**Примеры:** `Networking`, `Logging`, `DesignSystem`.
...
**Примеры:** `AuthAPI` (контракт токена, реализация — в `Auth`).
...
- **Разрыв циклов:** `Auth` зависит от `Networking`, а `Networking` нужен `Auth`, чтобы обновить токен → оба зависят от `AuthAPI`, реализация не циклится.
...
**Примеры:** `GameEngine` (Metal), `PurchasesSDK` (StoreKit).
