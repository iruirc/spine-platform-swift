...
| Scenario | Use TCA |
|---|---|
| Pure SwiftUI app, on any supported baseline (iOS 16 and macOS 13 through the Perception backport) | ✅ |
...
Key points:
- `@Bindable var store` (iOS 17+) or `@Perception.Bindable` (iOS 16 / macOS 13 backport, requires wrapping body in `WithPerceptionTracking { … }`).
