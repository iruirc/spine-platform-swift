...
The placement below holds in every mode. Two independent target settings change only how code spells it:

- **`SWIFT_APPROACHABLE_CONCURRENCY`** (it turns on `NonisolatedNonsendingByDefault`). Off: a `nonisolated async` body leaves the caller's actor at the `await`. On: the body runs on the caller's actor, and work that must not run there is marked `@concurrent`.
- **`SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`.** An unannotated type is main-actor-isolated, so UseCase, Repository, APIClient and Logger are marked `nonisolated` explicitly.

Xcode 26 project templates set both. A project from `swift-init` is generated by XcodeGen, which applies no Xcode template, so it sets neither.

## Per-Layer Isolation: One Table, All Five Architectures

| Architectural role | Isolation | Why | Where it appears |
|---|---|---|---|
| **View / ViewController** | `@MainActor` (implicit from the SDK, whatever the deployment target: `UIViewController`, and a SwiftUI `View` conformer since the iOS 18 SDK) | UIKit/SwiftUI APIs are main-actor-isolated. Don't fight it. | All architectures |
...
| **UseCase / Interactor (business logic)** | **nonisolated** | Pure orchestration, no UI state. Whether its `async` body leaves the caller's actor depends on the toolchain mode — see "Toolchain modes". | Clean (UseCase), VIPER (Interactor) |
...
3. **`UseCase`/`Interactor` is never `@MainActor`.** Mark it `nonisolated` where the target's default isolation is `MainActor`. With Approachable Concurrency off, the `await` from a `@MainActor` caller is the hop off main; with it on, the body stays on the caller's actor, and CPU-heavy work is marked `@concurrent`. A `@MainActor` UseCase puts business logic on the main thread in either mode.
...
struct LoadProfileScreen: UseCase {
    ...
    let posts: any PostsRepository
    let social: any SocialRepository

    func execute(userID: User.ID) async throws -> ProfileScreenData {
        async let profile = profiles.fetch(userID)
        async let recent = posts.recent(by: userID)
        async let followers = social.followers(of: userID)
        return try await ProfileScreenData(profile: profile, posts: recent, followers: followers)
    }
}
...
final class GalleryViewModel: ObservableObject {
    @Published private(set) var thumbnails: [URL: Data] = [:]   // not UIImage: the View decodes
    private let loader: any ThumbnailLoading

    init(loader: any ThumbnailLoading) {
        self.loader = loader
    }
    ...
    func loadThumbnails(_ urls: [URL]) async {
        await withTaskGroup(of: (URL, Data?).self) { group in
            for url in urls {
                group.addTask { (url, await self.loader.thumbnail(for: url)) }
            }
            for await (url, data) in group {
                thumbnails[url] = data    // safe — @MainActor
            }
        }
    }
}
...
final class ProfileRepository {
    func fetchScreenData(userID: User.ID) async throws -> ProfileScreenData {
        // Don't do this here: loading one screen from three sources is a business operation.
        // Repository exposes three separate methods; the UseCase composes them.
    }
}
```
...
| Errors | **Yes** — `Error` refines `Sendable` | Keep an error type's associated values and stored properties `Sendable`: Swift 6 warns on one that is not |
...
**Pattern:** the service lives in app scope (`di-composition-root` → "Scopes: app / scene / flow / request") and is injected into the screen.

<!-- typecheck -->
```swift
// Stateful app service owns its own Tasks
actor PhotoUploader {
    private let api: any PhotoUploadAPI
    private var inflight: [Photo.ID: Task<Void, Error>] = [:]

    init(api: any PhotoUploadAPI) {
        self.api = api
    }

    func enqueue(_ photo: Photo) {
        guard inflight[photo.id] == nil else { return }    // already uploading
...
final class ComposeViewModel {
    ...
    func tapSend(_ photo: Photo) {
        ...
        onSent?()                  // safe — uploader owns the Task
    }
}
```
...
1. **Singletons holding mutable shared state are actors.** Register them in app scope, spelled per container in `di-composition-root` → "Scopes: app / scene / flow / request". Resolution is `await`-free (the container itself doesn't need to be an actor) but every method call on the resolved instance is `await`.
